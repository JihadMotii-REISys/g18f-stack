...
Vagrant.configure('2') do |config|
  ...
  config.vm.provision :shell, run: 'always' do |s|
    ...
    #add provisioning SHELL script to be ran only once at the first vagrant up 
    config.vm.provision "shell" do |s|
        s.path = 'script/post-install.sh'

        #This script need param to install FULL STACK locally or Globally, 
        #as of now, let's install it as Locally and no need to set second param 
        #(as script request, because the path is always the same)
        s.args = ['no']
    end
